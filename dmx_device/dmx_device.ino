...
void setup() {
  ...
  //аFastLED.setMaxPowerInVoltsAndMilliamps(5, 1000);
...
void loop() {
  if (flag == 0) {
    ...
    if (sp > 0) {
      if (millis() - timing >= sp * 4) { // Вместо 10000 подставьте нужное вам значение паузы
        timing = millis();
        strobe != strobe;
        flag = 2;
        flag1 = 0;
  ...
  if (flag == 2) {
    FastLED.setBrightness(bright * strobe);
    FastLED.show();
    flag = 0;
    flag1 = 1;
  }
