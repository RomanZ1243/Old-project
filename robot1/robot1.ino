...
void loop() {
  if (flag == 0) {
    ...
    if (digitalRead(DL) == 1 && analogRead(D1) < black && analogRead(D2) < black && analogRead(D3) < black && analogRead(D4) < black && analogRead(D5) < black && analogRead(D6) < black&& analogRead(D7) < black && analogRead(D8) < black && digitalRead(DR) == 0) {
      k = -5;
      flag = 1;
    }
    if (digitalRead(DR) == 1 && analogRead(D1) < black && analogRead(D2) < black && analogRead(D3) < black && analogRead(D4) < black && analogRead(D5) < black && analogRead(D6) < black&& analogRead(D7) < black && analogRead(D8) < black && digitalRead(DL) == 0) {
      k = 5;
      flag = 1;
    }
    ...
    digitalWrite(L1, LOW);
    digitalWrite(R1, LOW);
    analogWrite(L2, mid_sp + (k * d));
    analogWrite(R2, mid_sp - (k * d));
  }
